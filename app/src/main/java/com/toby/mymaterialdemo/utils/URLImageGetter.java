...
public class URLImageGetter implements Html.ImageGetter {
    ...
    private static final String mFilePath = BaseApplication.getContext().getCacheDir().getAbsolutePath();
    ...
    private Drawable getDrawableFromNet(final String source) {
        AsyncHttpUtil.get(source, null, new AsyncHttpResponseHandler() {
            @Override
            public void onSuccess(int statusCode, Header[] headers, byte[] responseBody) {
                WritePicToDisk(ResponseBody.create(null, responseBody), source);
                mPicCount++;
                if (mPicCount == mPicTotal - 1) {
    ...
    private Drawable createPicPlaceholder() {
        ...
        drawable = new ColorDrawable(BaseApplication.getContext().getResources().getColor(color));
